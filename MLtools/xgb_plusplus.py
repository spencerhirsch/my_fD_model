...
class xgb:
    def __init__(self, dataset, file_name=None):
        ...
        if self.dataset == "mc":
            ...
            if file_name is None:
                print(
                    colors.RED + "The dataset name (e.g., file_name = qqToZZTo4L, file_name = DataAboveUpsilonCRSR_MZD_200_55_signal) must be provided!")
                sys.exit()
            else:
    ...
    def split(self, dataframe_shaped, test=0.25, random=7, scalerType=None, ret=False):
        ...
        X_train, X_test, y_train, y_test = train_test_split(X_data, Y_data, test_size=test,
                                                            random_state=random)  # split into train/test datasets

        self.trainX = pd.DataFrame(X_train,
                                   columns=["selpT0", "selpT1", "selpT2", "selpT3", "selEta0", "selEta1", "selEta2",
                                            "selEta3",
                                            "selPhi0", "selPhi1", "selPhi2", "selPhi3", "selCharge0", "selCharge1",
                                            "selCharge2", "selCharge3", "dPhi0", "dPhi1", "dRA0", "dRA1", "event",
                                            "invMassA0",
                                            "invMassA1"])
        # self.trainX = self.trainX.drop(['event'], axis = 1)

        self.testX = pd.DataFrame(X_test,
                                  columns=["selpT0", "selpT1", "selpT2", "selpT3", "selEta0", "selEta1", "selEta2",
                                           "selEta3",
                                           "selPhi0", "selPhi1", "selPhi2", "selPhi3", "selCharge0", "selCharge1",
                                           "selCharge2", "selCharge3", "dPhi0", "dPhi1", "dRA0", "dRA1", "event",
    ...
    def xgb(self, met=True, save=True, filename="MZD_200_55_pd_model.sav", single_pair=False, ret=True, verbose=True,
            saveFig=True):
        print("\n\n")
        print(60 * '*')
        ...
        if met:
            print("\n\n")
            print(60 * '*')
            print(colors.GREEN + "Retrieving metrics and plotting logloss and error" + colors.ENDC)
            print(60 * '*')

            ## plot the logloss and error figures
            eval_set = [(self.trainX, self.trainY), (self.testX, self.testY)]
            model.fit(self.trainX, self.trainY, early_stopping_rounds=10, eval_metric="logloss", eval_set=eval_set,
                      verbose=False)
            ...
            model.fit(self.trainX, self.trainY, early_stopping_rounds=10, eval_metric="error", eval_set=eval_set,
                      verbose=False)
            results = model.evals_result()
            ...
            if saveFig:
                plt.savefig(dataDir + ("/XGBClassError_default_plusplus_%s.pdf" % self.file_name), bbox_inches='tight')
    ...
    def predict(self, dataframe_shaped, filename="MZD_200_55_pd_model.sav", single_pair=False, ret=False, verbose=True):
        ...
        x_df = pd.DataFrame(x_data,
                            columns=["selpT0", "selpT1", "selpT2", "selpT3", "selEta0", "selEta1", "selEta2", "selEta3",
                                     "selPhi0", "selPhi1", "selPhi2", "selPhi3", "selCharge0", "selCharge1",
                                     "selCharge2", "selCharge3", "dPhi0", "dPhi1", "dRA0", "dRA1", "event", "invMassA0",
                                     "invMassA1"])
        ...
        if verbose:
            ...
            print(60 * '*')
            print(
                colors.GREEN + "Determining correct and wrong pairs for the %s dataset" % self.file_name + colors.ENDC)
            print(60 * '*')
    ...
    def plotMatch(self, save=True, verbose=False):
        ...
        for pair in range(2):
            if pair == 0:
                self.correct_pair.plot(x='invMassA0', y='invMassA1', kind='scatter', color='darkred', ax=ax[pair],
                                       zorder=3)
                ax[pair].set_title('Correct Pair')
                ax[pair].set_ylim(0, 80)
                ax[pair].set_xlim(0, 80)
            elif pair == 1:
                self.wrong_pair.plot(x='invMassA0', y='invMassA1', kind='scatter', color='darkred', ax=ax[pair],
                                     zorder=3)
        ...
        for pair in range(4):
            if pair <= 1:
                ...
                self.correct_pair[key].plot.hist(bins=100, alpha=0.9, range=(0, 100), color='darkred', ax=ax[pair],
                                                 zorder=3)
                ax[pair].set_title('Correct Pair')
            elif pair > 1:
                if pair == 2:
                ...
                self.wrong_pair[key].plot.hist(bins=100, alpha=0.9, range=(0, 200), color='darkred', ax=ax[pair],
                                               zorder=3)
                ax[pair].set_title('Wrong Pair')
